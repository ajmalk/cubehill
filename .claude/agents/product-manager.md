...
**D2 is the preferred tool** for technical diagrams embedded in docs. Create `.d2` source files in `docs/*/diagrams/`, render to SVG in `docs/*/images/`, and embed in markdown. See `docs/process/contributing.md` for the full workflow. Mermaid is still fine for simple inline diagrams; FigJam for brainstorming and spatial work.

When creating or editing diagrams in docs:

1. **Verify rendered output**: After adding or modifying a diagram, visually inspect the rendered SVG (or use Playwright browser tools for Mermaid on GitHub). Check for overlapping labels, excessive width, unbalanced spacing, or unreadable text.
...
3. **Commit source and output**: For D2 diagrams, commit both the `.d2` source file and the rendered `.svg`. Add a source note below the embedded image.
4. **Avoid multi-line labels**: Multi-line strings in nodes and edges cause layout problems. Use them sparingly and only when essential.
5. **Minimize subgraphs**: Each subgraph adds width. Prefer flat graphs with styling over nested subgraphs.
6. **Iterate on issues**: If the rendered diagram has visual problems, fix and re-check. Never merge a diagram you haven't visually verified.
